...
import { Files, Copy, Globe, Eye, Server, RefreshCw } from "lucide-react";
...
export default function FilesPage() {
  ...
  const { data, isLoading, refetch, isFetching } = useAdminFiles(page, limit);
  ...
  // If Admin role, render Global Platform Files explorer
  if (isAdmin) {
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[13px] text-ink-muted mt-0.5">
              Review and audit all user uploads stored across providers.
            </p>
          </div>
          <Button
            size="sm"
        ...
        <Card className="bg-bg-surface border-border-strong">
          <CardHeader className="pb-3 flex flex-row items-center justify-between space-y-0">
            <CardTitle className="text-sm font-semibold">All User Files ({total})</CardTitle>
            <Input
              placeholder="Filter by filename or owner..."
              value={search}
          ...
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <div className="p-6 text-sm text-ink-muted text-center">No files found.</div>
            ) : (
  ...
  // Standard User Workspace Explorer
