...
import { ChevronLeft, ArrowUp, ArrowDown, Activity, Clock, Server, FileCode2 } from "lucide-react";
  ...
  if (!transfer) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        <p className="text-sm text-ink-muted">Transfer tracking session not found.</p>
        <Link href="/transfers" className="mt-4 text-xs text-accent hover:underline">
  ...
  return (
    <div className="flex flex-col gap-6 font-sans">
      <div className="flex items-center justify-between">
        <Link href="/transfers" className="inline-flex items-center gap-1.5 text-[13px] text-ink-muted hover:text-ink transition-colors">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          <Card className="bg-bg-surface border-border-strong text-ink p-5 flex flex-col gap-5">
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0 flex-1">
                <h2 className="text-base font-semibold leading-none truncate text-ink">{transfer.fileName}</h2>
                <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-ink-muted">
                  <span className="flex items-center gap-1"><FileCode2 className="h-3.5 w-3.5" /> {formatBytes(transfer.sizeBytes)}</span>
                  <span className="flex items-center gap-1"><Server className="h-3.5 w-3.5" /> Object Storage</span>
                </div>
              </div>
            </div>
          ...
          <Card className="bg-bg-surface border-border-strong text-ink p-5">
            <CardHeader className="p-0 pb-3 flex flex-row items-center justify-between border-b border-border/50">
              <CardTitle className="text-xs font-mono uppercase tracking-wider text-ink-muted">Chunk Processing Matrix</CardTitle>
              <span className="text-[11px] text-ink-faint font-mono">
                {transfer.chunks.filter(c => c.status === "complete").length} / {transfer.totalChunks} Chunks Verified
              </span>
            </CardHeader>
            <div className="pt-5">
              <ChunkVisualizer chunks={transfer.chunks} totalChunks={transfer.totalChunks} className="gap-[4px] justify-center" />
              <div className="mt-5 grid grid-cols-2 gap-4 border-t border-border/50 pt-4 font-mono text-[11px] text-ink-faint sm:grid-cols-4">
                <div className="flex items-center gap-1.5">
                  <span className="h-2 w-2 rounded-full bg-accent" /> Completed Chunk
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="h-2 w-2 rounded-full bg-live animate-pulse-live" /> Streaming Chunk
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="h-2 w-2 rounded-full bg-danger" /> Spoofed Signature Block
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="h-2 w-2 rounded-full bg-bg-overlay" /> Queued / Pending
                </div>
              </div>
            </div>
          </Card>
          ...
          {transfer.status === "active" && transfer.speedHistory.length > 0 && (
            <Card className="bg-bg-surface border-border-strong text-ink p-5">
              <CardHeader className="p-0 pb-3 border-b border-border/50">
                <CardTitle className="text-xs font-mono uppercase tracking-wider text-ink-muted">Real-time Bandwidth (Last 30s)</CardTitle>
              </CardHeader>
              <div className="pt-5">
        ...
        <div className="lg:col-span-1">
          <Card className="bg-bg-surface border-border-strong text-ink p-5 h-full">
            <CardHeader className="p-0 pb-3 flex flex-row items-center gap-2 border-b border-border/50">
              ...
              <CardTitle className="text-xs font-mono uppercase tracking-wider text-ink-muted">Execution Timeline Logs</CardTitle>
            </CardHeader>
            <div className="pt-5 max-h-[500px] overflow-y-auto">
              {transfer.logs.length === 0 ? (
                <p className="text-[12px] text-ink-faint">No logs registered yet.</p>
              ) : (
