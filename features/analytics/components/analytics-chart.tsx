    ...
    return (
      <ResponsiveContainer width="100%" height={260}>
        <LineChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
          ...
          <XAxis dataKey="date" tickFormatter={(d) => formatDate(d)} stroke="#857C72" fontSize={11} tickLine={false} axisLine={false} minTickGap={30} />
          <YAxis domain={[90, 100]} stroke="#857C72" fontSize={11} tickLine={false} axisLine={false} width={40} />
          <Tooltip
    ...
    return (
      <ResponsiveContainer width="100%" height={260}>
        <AreaChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
          ...
          <CartesianGrid stroke="#241F1B" vertical={false} />
          <XAxis dataKey="date" tickFormatter={(d) => formatDate(d)} stroke="#857C72" fontSize={11} tickLine={false} axisLine={false} minTickGap={30} />
          <YAxis tickFormatter={(v) => formatBytes(v)} stroke="#857C72" fontSize={11} tickLine={false} axisLine={false} width={70} />
          <Tooltip
            contentStyle={{ background: "#1A1714", border: "1px solid #382F28", borderRadius: 8, fontSize: 12 }}
            labelFormatter={(d) => formatDate(d)}
          ...
          <Area type="monotone" dataKey="uploadBytes" stroke="#FF6A00" strokeWidth={2} fill="url(#upBytes)" />
  ...
  return (
    <ResponsiveContainer width="100%" height={260}>
      <AreaChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
        ...
        <Area type="monotone" dataKey="uploads" stroke="#FF6A00" strokeWidth={2} fill="url(#upCount)" />
